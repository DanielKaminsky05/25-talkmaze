...
import { setProfileCookies } from "@/src/lib/profile-management/profile-cookies";
    ...
    for (const startLocal of potentialStarts) {
      ...
      for (const coach of shuffledCoaches) {
        ...
        if (!coachCollision) {
          ...
          if (!studentCollision) {
            console.log(
              `[MATCHMAKER] ✅ PERFECT MATCH! Coach ${coach.coach_id} assigned.`,
            );
...
function startDateForBookedSlot(
  ...
  return (
    bookedSlot.start_date || nextMatchingDateForWeekday(bookedSlot.weekday)
  );
}
