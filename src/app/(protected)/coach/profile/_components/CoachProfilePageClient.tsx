...
import { updateCoachInfo } from "../actions";
...
export default function CoachProfilePageClient({ coach, accountEmail }: Props) {
  return (
      ...
      <main className="bg-[#1f2e3b] rounded-3xl mx-8 mb-10 flex-1 px-10 py-8 xl:px-16">
        ...
        <div className="flex flex-col lg:flex-row gap-8 items-stretch lg:items-start">
          ...
          <div className="flex-1 flex flex-col gap-6 min-w-0">
            <PersonalSection coach={coach} />
            <ContactSection accountEmail={accountEmail} />
          </div>
        </div>
      </main>
    </div>
...
// ---------------------------------------------------------------------------
// Left Panel - Avatar, Bio, Location, Specialty
// ---------------------------------------------------------------------------

function LeftPanel({ coach }: { coach: CoachData }) {
  ...
  async function handleConfirmUpload() {
    ...
    const path = `${coach.id}/avatar-${Date.now()}.${ext}`;
    ...
    const result = await updateCoachInfo(coach.id, { avatar_url: publicUrl });
    setUploading(false);
  ...
  async function handleSave() {
    setSaving(true);
    setError(null);
    const result = await updateCoachInfo(coach.id, {
      bio,
      location,
      specialty,
    });
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {/* Bio, Location, Specialty - read/edit toggle */}
        ...
        {editing ? (
          <div>
            ...
            <input
              ...
              placeholder="e.g. Competitive Speech"
              className="w-full bg-[#2b4257] text-white text-sm border border-[#B1E7D6]/40 rounded-lg px-3 py-2 placeholder-gray-500 focus:outline-none focus:border-[#B1E7D6] transition-colors"
            />
          </div>
        ) : (
          specialty && (
            <p className="text-[#B1E7D6] text-sm italic">{specialty}</p>
          )
        )}
...
function PersonalSection({ coach }: { coach: CoachData }) {
  const [editing, setEditing] = useState(false);
...
// ---------------------------------------------------------------------------
// Contact Section (read-only email)
// ---------------------------------------------------------------------------

function ContactSection({ accountEmail }: { accountEmail: string }) {
  return (
