...
export async function GET(
  ...
  try {
    const { data, error } = await supabase
      ...
      .select("weekday, start_time, end_time, start_time_new, end_time_new, timezone")
      .eq("coach_id", parsed.data.id);

    if (error) {
      console.error("admin/employees/[id]/availability GET error", error);
      return NextResponse.json(
    ...
    console.error("admin/employees/[id]/availability GET error", err);
    return NextResponse.json(
      { error: "Internal server error" },
      { status: 500 },
    );
...
export async function PUT(
  ...
  try {
    ...
    if (deleteError) {
      console.error(
        "admin/employees/[id]/availability PUT delete error",
        deleteError,
      );
      return NextResponse.json(
        { error: "Internal server error" },
    ...
    if (rows.length > 0) {
      ...
      if (insertError) {
        console.error(
          "admin/employees/[id]/availability PUT insert error",
          insertError,
        );
        return NextResponse.json(
          { error: "Internal server error" },
    ...
    console.error("admin/employees/[id]/availability PUT error", err);
    return NextResponse.json(
      { error: "Internal server error" },
      { status: 500 },
    );
