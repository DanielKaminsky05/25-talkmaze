...
export async function GET() {
  ...
  try {
    ...
    if (error) {
      console.error("admin/payment-plans GET error", error);
    ...
    console.error("admin/payment-plans GET error", err);
    return NextResponse.json(
      { error: "Internal server error" },
      { status: 500 },
    );
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (error) {
      console.error("admin/payment-plans POST insert error", error);
      return NextResponse.json(
        { error: "Internal server error" },
        { status: 500 },
      );
    ...
    console.error("admin/payment-plans POST error", err);
    return NextResponse.json(
      { error: "Internal server error" },
      { status: 500 },
    );
