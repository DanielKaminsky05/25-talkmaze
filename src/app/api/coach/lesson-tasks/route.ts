...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    if (fileBlob) {
      ...
      if (uploadError) {
        console.error("coach/lesson-tasks upload error", uploadError);
        return NextResponse.json(
    ...
    if (existing) {
      const { data, error } = await supabase
        ...
        .single();
      if (error) {
        console.error("coach/lesson-tasks update error", error);
        return NextResponse.json(
          { error: "Internal server error" },
          { status: 500 },
        );
      ...
      const { data, error } = await supabase
        ...
        .single();
      if (error) {
        console.error("coach/lesson-tasks insert error", error);
        return NextResponse.json(
          { error: "Internal server error" },
          { status: 500 },
        );
    ...
    console.error("coach/lesson-tasks error", err);
    return NextResponse.json(
      { error: "Internal server error" },
      { status: 500 },
    );
