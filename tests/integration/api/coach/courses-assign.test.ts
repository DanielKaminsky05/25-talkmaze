/**
 * Contract tests for POST /api/coach/courses/assign.
 *
 * Coach-scoped equivalent of /api/admin/courses/assign — same body, same
 * { success: true } response, same side effects. The difference is auth:
 * coach role (2) + coach_students ownership of the student.
 *
 * Five questions:
...
import { POST as assignPOST } from "@/src/app/api/coach/courses/assign/route";
...
// Q1
describe("POST /api/coach/courses/assign — ownership", () => {
  it("returns 403 when the coach is not linked to the target student", async () => {
    ...
    const res = await call(assignPOST, {
      method: "POST",
      cookies,
      body: { studentId: otherStudent.id, courseId: course.id },
    });
...
// Q2
describe("POST /api/coach/courses/assign — input validation", () => {
  it("returns 400 when studentId is not a UUID", async () => {
    const { cookies, course } = await seedCoachWithLinkedStudentAndCourse();
    const res = await call(assignPOST, {
      method: "POST",
      cookies,
      body: { studentId: "not-a-uuid", courseId: course.id },
    });
  ...
  it("returns 400 when courseId is not a UUID", async () => {
    ...
    const res = await call(assignPOST, {
      method: "POST",
      cookies,
      body: { studentId: student.id, courseId: "not-a-uuid" },
    });
    expect(res.status).toBe(400);
  });
  ...
  it("returns 400 when body contains unknown fields (strict)", async () => {
    ...
    const res = await call(assignPOST, {
      method: "POST",
      cookies,
      body: { studentId: student.id, courseId: course.id, extraneous: "field" },
    });
    expect(res.status).toBe(400);
  });
...
// Q3
describe("POST /api/coach/courses/assign — response shape", () => {
...
// Q4
describe("POST /api/coach/courses/assign — side effects", () => {
