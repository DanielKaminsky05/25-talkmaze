/**
 * Contract tests for GET /api/coach/conversation.
 *
 * Upserts a conversation between the calling coach and the contact (student
 * or parent) and returns the conversation id. The "?contactId=" is the
...
// Q2
describe("GET /api/coach/conversation — input validation", () => {
  it("returns 400 when contactId is missing", async () => {
    const { cookies } = await seedCoachAndStudent();
    const res = await call(conversationGET, { cookies });
...
// Q3
describe("GET /api/coach/conversation — response shape", () => {
...
// Q4
describe("GET /api/coach/conversation — side effects", () => {
  ...
  it("is idempotent — repeated calls return the same conversationId (upsert)", async () => {
    const { cookies, student } = await seedCoachAndStudent();
    const first = await call(conversationGET, {
      cookies,
      query: { contactId: student.id },
    });
    const second = await call(conversationGET, {
      cookies,
      query: { contactId: student.id },
    });
