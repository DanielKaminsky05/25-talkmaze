/**
 * Contract tests for GET /api/coach/conversation/message.
 *
 * Returns the messages in a conversation, enriched with sender name + avatar.
 * Read-only.
 *
 * Five questions:
 *   Q1 ownership — assertCoachOwnsConversation(conversationId)
 *   Q2 validation — conversationId required + UUID + .strict()
...
import { GET as messageGET } from "@/src/app/api/coach/conversation/message/route";
...
describe("GET /api/coach/conversation/message — ownership", () => {
  it("returns 403 when coach does not own the conversation", async () => {
    ...
    const res = await call(messageGET, {
  ...
  it("returns 404 when the conversation does not exist", async () => {
    ...
    const res = await call(messageGET, {
      cookies,
      query: { conversationId: "00000000-0000-0000-0000-000000000000" },
    });
...
// Q2
describe("GET /api/coach/conversation/message — input validation", () => {
  it("returns 400 when conversationId is missing", async () => {
    const { cookies } = await seedConversationWithMessage();
    const res = await call(messageGET, { cookies });
    expect(res.status).toBe(400);
  });

  it("returns 400 when conversationId is malformed (not a UUID)", async () => {
    const { cookies } = await seedConversationWithMessage();
    const res = await call(messageGET, {
      cookies,
      query: { conversationId: "not-a-uuid" },
    });
    expect(res.status).toBe(400);
  });
...
// Q3
describe("GET /api/coach/conversation/message — response shape", () => {
