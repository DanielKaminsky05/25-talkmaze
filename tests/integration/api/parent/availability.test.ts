/**
 * Contract tests for /api/parent/students/[studentId]/availability (GET, PUT).
 *
...
import {
  GET,
  PUT,
} from "@/src/app/api/parent/students/[studentId]/availability/route";
...
describe("GET /api/parent/students/[studentId]/availability — ownership", () => {
...
describe("GET /api/parent/students/[studentId]/availability — input validation", () => {
...
describe("GET /api/parent/students/[studentId]/availability — response shape", () => {
...
describe("PUT /api/parent/students/[studentId]/availability — ownership", () => {
  it("returns 403 when caller does not own the student", async () => {
    const { student } = await seedOwnerWithStudent();
    const other = await createAccount({ role: 1 });
    const otherCookies = await signSessionFor(other);
...
describe("PUT /api/parent/students/[studentId]/availability — input validation", () => {
  it("returns 400 when studentId is not a UUID", async () => {
    const { cookies } = await seedOwnerWithStudent();
    const res = await call(PUT, {
      method: "PUT",
