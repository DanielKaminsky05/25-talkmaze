/**
 * Contract tests for POST /api/subscriptions/cancel.
 *
...
describe("POST /api/subscriptions/cancel — ownership", () => {
...
describe("POST /api/subscriptions/cancel — input validation", () => {
  ...
  it("returns 400 when refund is a non-boolean", async () => {
    ...
    const res = await call(cancelPOST, {
      method: "POST",
      cookies,
      body: { studentId: student.id, refund: "yes" },
    });
    expect(res.status).toBe(400);
  });
...
describe("POST /api/subscriptions/cancel — response shape", () => {
  it("returns { success: true } on schedule-at-period-end", async () => {
    ...
    const res = await call(cancelPOST, {
      method: "POST",
      cookies,
      body: { studentId: student.id, refund: false },
    });
  ...
  it("returns { success: true } on immediate refund + cancel", async () => {
    ...
    const res = await call(cancelPOST, {
      method: "POST",
      cookies,
      body: { studentId: student.id, refund: true },
    });
    expect(res.status).toBe(200);
    const body = await res.json<{ success: boolean }>();
    expect(body).toEqual({ success: true });
...
describe("POST /api/subscriptions/cancel — side effects", () => {
...
describe("POST /api/subscriptions/cancel — external calls", () => {
