/**
 * Contract tests for POST /api/subscriptions/schedule.
 *
...
describe("POST /api/subscriptions/schedule — ownership", () => {
  it("returns 403 when the caller does not own the target student", async () => {
    ...
    const res = await call(schedulePOST, {
      ...
      body: { studentId: student.id, priceId: target.stripe_price_id },
    });
...
describe("POST /api/subscriptions/schedule — input validation", () => {
  ...
  it("returns 400 when studentId is malformed (not a UUID)", async () => {
    ...
    const res = await call(schedulePOST, {
      method: "POST",
      cookies,
      body: { studentId: "not-a-uuid", priceId: target.stripe_price_id },
    });
    expect(res.status).toBe(400);
  });
...
describe("POST /api/subscriptions/schedule — response shape", () => {
...
describe("POST /api/subscriptions/schedule — side effects", () => {
  ...
  it("returns 404 when the target priceId does not match any plan", async () => {
    ...
    const res = await call(schedulePOST, {
      method: "POST",
      cookies,
      body: { studentId: student.id, priceId: "price_does_not_exist_xyz" },
    });
    expect(res.status).toBe(404);
    expect(stripe.setupIntents.create).not.toHaveBeenCalled();
  });
  ...
  it("returns 409 when the student is already on the requested plan", async () => {
    ...
    const res = await call(schedulePOST, {
      method: "POST",
      cookies,
      body: { studentId: student.id, priceId: plan.stripe_price_id },
    });
...
describe("POST /api/subscriptions/schedule — external calls", () => {
  ...
  it("passes the existing pending schedule id through metadata.replace_schedule_id when one exists", async () => {
    ...
    await call(schedulePOST, {
      method: "POST",
      cookies,
      body: { studentId: student.id, priceId: newTarget.stripe_price_id },
    });
