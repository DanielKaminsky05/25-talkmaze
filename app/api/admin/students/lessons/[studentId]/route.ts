...
export async function GET(
    ...
    const {data: assigned_courses, error: assigned_courses_error} = await supabase.from('course_assignment').select('course_id').eq('student_id',studentId)

    if(assigned_courses_error){
        console.log("error getting assigned courses ", assigned_courses_error);
        return NextResponse.json({status:500, message: "Error retrieving courses assigned to student"})
    }

    console.log("Retrieved assigned courses: " + JSON.stringify(assigned_courses));
    ...
    for(let i = 0; i < assigned_courses.length; i++){
        ...
        const {data: courseHeadData, error: courseHeadError} = await supabase.from('courses').select('head_lesson_id,title').eq('id',course_id).single();

        console.log("Retrieved Head: " + courseHeadData?.head_lesson_id)
        //Get all the lessons of the course

        const{data: allLessonsData, error: allLessonsError} = await supabase.from('lessons').select("*").eq('course_id', course_id)
        
        
        if(allLessonsError){
            return NextResponse.json("Error getting all lessons of associated course " + course_id)
        }

        //get the statuses of the lessons
        const {data: statusData, error: statusDataError} = await supabase.from('lesson_progress').select('*').eq('student_id',studentId);

        if(statusDataError){
            return NextResponse.json({status:500, message: "Unable to fetch the statuses of the lessons"})
        }
        ...
        const head_lesson = allLessonsData?.find(lesson => lesson.id === courseHeadData?.head_lesson_id)
        
        let current_lesson = head_lesson;

        while(current_lesson){
            const status = statusData.find(statusObj => statusObj.lesson_id ===  current_lesson?.id)?.status;
            ...
            current_lesson = allLessonsData.find(lesson=>lesson.id === next_lesson_id)
        }
