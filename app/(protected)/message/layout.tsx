 ...
 * - Coaches/admins: see all other accounts
...
async function getContacts(): Promise<Contact[]> {
    ...
    const coachAccountIds = data
        .filter((a) => a.role === 2)
        .map((a) => a.id);

    const { data: coaches } = coachAccountIds.length > 0
        ? await supabase
              .from("coaches")
    ...
    const coachNameMap = new Map((coaches ?? []).map((c) => [c.account_id, c.name]));
