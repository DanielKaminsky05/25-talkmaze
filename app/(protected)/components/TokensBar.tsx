...
function tokenClass(index: number, completedCount: number) {
  return index < completedCount ? "" : "opacity-30";
}

export default function TokenBar({ completedCount }: TokenBarProps) {
  return (
    <div
      className="relative bg-white rounded-xl shadow-[0px_4px_4px_rgba(0,0,0,0.25)] box-border overflow-visible"
      style={{
        width: "402px",
        height: "147px",
        flexShrink: 0,
    ...
    >
      ...
      <div className="absolute left-[37px] top-[66px] flex items-start justify-start gap-4">

        {/* COMPASS — index 0 */}
        <div data-token="compass" className={`relative w-[33px] h-[33px] overflow-hidden shrink-0 ${tokenClass(0, completedCount)}`}>
        ...
        {/* SPYGLASS — index 1 */}
        <div data-token="spyglass" className={`relative w-[33px] h-[33px] overflow-hidden shrink-0 ${tokenClass(1, completedCount)}`}>
          <div className="absolute left-[1.35px] top-[1.34px] w-[30.55px] h-[30.43px] bg-[#604C3D]" />
        </div>

        {/* SAND DOLLAR — index 2 */}
        <div data-token="sand-dollar" className={`relative w-[33px] h-[33px] overflow-hidden shrink-0 ${tokenClass(2, completedCount)}`}>
        ...
        {/* ARROW — index 3 */}
        <div data-token="arrow" className={`relative w-[33px] h-[33px] overflow-hidden shrink-0 ${tokenClass(3, completedCount)}`}>
          <div className="absolute left-[8.31px] top-[1.32px] w-[19px] h-[30.36px] bg-[#5A50CD]" />
        </div>

        {/* PINEAPPLE — index 4 */}
        <div data-token="pineapple" className={`relative w-[33px] h-[33px] overflow-hidden shrink-0 ${tokenClass(4, completedCount)}`}>
        ...
        {/* MAP — index 5 */}
        <div data-token="map" className={`relative w-[34px] h-[33px] overflow-hidden shrink-0 ${tokenClass(5, completedCount)}`}>
          <div className="absolute left-[2.28px] top-[2.24px] w-[29.44px] h-[28.53px] bg-[#185476]" />
        </div>

        {/* SEASHELL — index 6 */}
        <div data-token="seashell" className={`relative w-[33px] h-[33px] overflow-hidden shrink-0 ${tokenClass(6, completedCount)}`}>
