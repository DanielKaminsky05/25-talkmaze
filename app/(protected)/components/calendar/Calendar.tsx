...
export default function Calendar() {
    ...
    const handleSelect = (y: number, m: number, d: number) => {
        ...
        const isAlreadySelected = isSelected(y, m, d);
            ...
            <div className="w-full mb-2 grid gap-y-2 justify-between grid-cols-[repeat(7,max-content)]">
                ...
                {[...Array(firstDayOfMonth).keys()].map((_, index) => {
                    ...
                    return (
                        <span
                            key={`empty-${index}`}
                            className={`${inter.className} flex w-20 h-20 items-center justify-center text-[22px] font-bold rounded-xl shadow-[inset_0_4px_4px_rgba(0,0,0,0.25)] cursor-pointer ${isSelected(prevMonthYear, prevMonthIndex, prevDay)
                ...
                {[...Array(daysInMonth).keys()].map((day) => {
                    ...
                    return (
                        <span
                            key={dayNum}
                            className={`${inter.className} relative flex w-20 h-20 items-center justify-center text-[22px] font-bold rounded-xl shadow-[inset_0_4px_4px_rgba(0,0,0,0.25)] cursor-pointer ${isSelected(currentYear, currentMonth, dayNum)
                                ? "bg-[#B1E7D6] text-[#142535]"
                                : "bg-[#142535] text-[#65CFAD]"
                                }`}
                ...
                {[...Array(emptySpacesNextMonth).keys()].map((_, index) => {
                    ...
                    return (
                        <span
                            key={`next-${index}`}
                            className={`${inter.className} flex w-20 h-20 items-center justify-center text-[22px] font-bold rounded-xl shadow-[inset_0_4px_4px_rgba(0,0,0,0.25)] cursor-pointer ${isSelected(nextMonthYear, nextMonthIndex, nextDay)
                                ? "bg-[#B1E7D6] text-[#142535]"
                                : "bg-[#142535] text-[#65CFAD]"
                                }`}
                            onClick={() =>
